...
class TerrenoEditPrivate extends Component {
                            ...
                            <Tooltip title="Quitar">
                                <Button shape="circle" style={ { marginRight: 5, } }
                                    ...
                                    onClick={ ( evt ) => {
                                        ...
                                        this.state.arrayDeleteFKIDServicio = [ ...this.state.arrayDeleteFKIDServicio, record ];
                                        this.setState( {
                                            arrayFKIDServicio: this.state.arrayFKIDServicio,
                                            arrayDeleteFKIDServicio: this.state.arrayDeleteFKIDServicio,
                                        } );
                                    } }
                                />
                            </Tooltip>
    ...
    get_data( ) {
        axios.get( "/api/terreno/edit/" + this.props.params.idterreno ) . then ( ( resp ) => {
            console.log(resp)
            if ( resp.data.rpta === 1 ) {
                this.setState( {
    ...
    onAgregarServicio() {
        if ( this.state.fkidservicio === "" ) {
            Swal.fire( {
                position: 'top-end',
                icon: 'warning',
                title: "Favor de seleccionar Especialidad.",
                showConfirmButton: false,
                timer: 1500
            } );
            return;
    ...
    onStore() {
        let body = {
            ...
            idterreno: this.props.params.idterreno,

            arrayFKIDServicio: JSON.stringify( this.state.arrayFKIDServicio ),
            arrayDeleteFKIDServicio: JSON.stringify( this.state.arrayDeleteFKIDServicio ),
        };
        this.setState( { disabled: true, } );
        axios.post( "/api/terreno/update", body ) . then ( ( resp ) => {
            console.log(resp)
    ...
    render() {
        return (
            <>
                <Card title={"EDITAR TERRENO"} bordered 
                    extra={ 
